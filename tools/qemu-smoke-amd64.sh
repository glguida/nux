#!/bin/sh
# Run the reviewed amd64 QEMU serial-marker smoke test from an out-of-tree build.
#
# Environment:
#   TOOLBIN           Optional path to i686-unknown-elf toolchain bin directory.
#   TOOLCHAIN         64-bit target tool prefix, default x86_64-linux-gnu.
#   TOOLCHAIN32       32-bit APXH multiboot compiler prefix, default i686-unknown-elf.
...
#   QEMU_LOG          QEMU serial/output log path, default $BUILD/qemu-amd64-smoke.log.
...
if [ -n "${BUILD:-}" ]; then
    ...
    build_input=${TMPDIR:-/tmp}/the-nux-amd64-smoke-$$
fi
...
configure_log=${CONFIGURE_LOG:-$builddir/configure-amd64.log}
make_log=${MAKE_LOG:-$builddir/make-amd64.log}
qemu_log=${QEMU_LOG:-${LOG:-$builddir/qemu-amd64-smoke.log}}
...
if [ -n "${TOOLBIN:-}" ]; then
    ...
    note "toolbin: not set; using $TOOLCHAIN32 tools already on PATH"
fi
note "toolchain: $TOOLCHAIN"
note "toolchain32: $TOOLCHAIN32"
