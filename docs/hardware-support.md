...
| `amd64` | Yes (`configure.ac`) | `libhal_x86` amd64 | `libplt_acpi` | `apxh/configure` selects `multiboot efi` | Source has amd64 HAL, multiboot, EFI, ACPI, LAPIC/IOAPIC/HPET support. In this container, default configure is blocked on missing `amd64-unknown-elf-*`; an `x86_64-linux-gnu` prefix override with the reviewed i686 `TOOLCHAIN32` passes preflight/configure/build after the freestanding libec `-no-pie` fix. Bounded QEMU reaches APXH/NUX/tests, then hits the known `Unexpected Kernel Page Fault`; this is partial runtime coverage, not a full amd64 smoke pass. |
...
- EFI support still needs full architecture-specific runtime verification. i386 EFI source/settings are present but not selected by APXH configure; amd64 override preflight/configure/build now passes and partially boots under QEMU before a known kernel page fault; riscv64 target-toolchain and QEMU coverage remain open. APXH subdir selection is no longer blocked by the malformed generated configure case.
...
| `amd64` | `/usr/bin/qemu-system-x86_64` is present from the same package, QEMU `10.0.8 (Debian 1:10.0.8+ds-0+deb13u1+b2)` | Partially verified. Default `ARCH=amd64` configure fails at missing default `amd64-unknown-elf-*` tools; `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` passes preflight/configure/build with the stable external i686 `TOOLBIN` after the libec `-no-pie` fix. Bounded QEMU reaches APXH/NUX boot output plus `UADDR_VALIDRANGE test passed` and `KVA_ALLOC_FREE test passed`, then hits the known `Unexpected Kernel Page Fault` before timeout. |
