...
- x86 user-access windows are SMAP-aware: CPUID leaf 7 gates per-CPU `CR4.SMAP` enablement, `hal_useraccess_start/end` use `stac`/`clac` only after SMAP is enabled locally, and user-origin entry clears AC when SMAP is active. `CR4.SMEP` is not enabled yet and remains a separate hardening audit.
