...
| `amd64` | Yes (`configure.ac`) | `libhal_x86` amd64 | `libplt_acpi` | `apxh/configure` selects `multiboot efi` | Source has amd64 HAL, multiboot, EFI, ACPI, LAPIC/IOAPIC/HPET support. In this container, default configure is blocked on missing `amd64-unknown-elf-*`; an `x86_64-linux-gnu` prefix override configures successfully with the reviewed i686 `TOOLCHAIN32`, but full build/runtime verification is still blocked by submodule/workspace capacity. |
...
| `amd64` | `/usr/bin/qemu-system-x86_64` is present from the same package, QEMU `10.0.8 (Debian 1:10.0.8+ds-0+deb13u1+b2)` | Partially verified. Default `ARCH=amd64` configure fails at `amd64-unknown-elf-gcc not found`; `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` configures through top-level/APXH/example with the stable i686 `TOOLBIN`, but the build stops before QEMU because submodule-dependent paths cannot be completed in the full workspace. |
