...
| `amd64` | Yes (`configure.ac`) | `libhal_x86` amd64 | `libplt_acpi` | `apxh/configure` selects `multiboot efi` | Source has amd64 HAL, multiboot, EFI, ACPI, LAPIC/IOAPIC/HPET support. Default configure is blocked on missing `amd64-unknown-elf-*`; the standardized local smoke path is the reviewed `x86_64-linux-gnu` prefix override with the stable i686 `TOOLCHAIN32`. `tools/qemu-smoke-amd64.sh` now captures that path in a checked-in out-of-tree harness. Bounded QEMU reaches APXH/NUX/tests, IPI, userspace, syscall, `UCTXT_SETA2`, exit, and idle counter markers; timeout rc 124 is expected after success because the demo keeps idling. |
| `riscv64` | Yes (`configure.ac`) | `libhal_riscv` | `libplt_sbi` | `apxh/configure` selects `sbi efi`; EFI RISC-V code exists in `apxh/efi/apxhefi/efi_md.c` | SBI/DTB is the coherent configured path. Default configure is blocked on missing `riscv64-unknown-elf-*`; RISC-V QEMU is also absent here, and RISC-V EFI platform-descriptor compatibility remains unverified. |
...
- EFI support still needs full architecture-specific runtime verification. i386 EFI source/settings are present but not selected by APXH configure; amd64 multiboot override preflight/configure/build and bounded QEMU now pass with the reviewed `x86_64-linux-gnu`/`i686-unknown-elf` override after the libec PIE fixes, but amd64 EFI-specific runtime verification and the default amd64 target-toolchain path remain open; riscv64 target-toolchain and QEMU coverage remain open. APXH subdir selection is no longer blocked by the malformed generated configure case.
...
| `amd64` | `/usr/bin/qemu-system-x86_64` is present from the same package in the reviewed QEMU environment, QEMU `10.0.8 (Debian 1:10.0.8+ds-0+deb13u1+b2)` | Override runtime verified; default target-toolchain still blocked. Default `ARCH=amd64` configure fails at missing default `amd64-unknown-elf-*` tools and default-PATH `i686-unknown-elf-gcc`; `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` is the standardized local smoke path with the stable external i386 `TOOLBIN`. `tools/qemu-smoke-amd64.sh` now checks that path and requires APXH/NUX boot output, `IPI!`, userspace hello, `SYSC0`-`SYSC6`, `UCTXT_SETA2` kernel/user markers, `User exited with error code: 42`, no unexpected kernel page fault, and repeated zero-valued `pnux_entry_pagefault` idle counter lines before accepting timeout rc 124. |
| `riscv64` | Source target uses `qemu-system-riscv64 -M virt`, but `qemu-system-riscv64` is not on `PATH` in this container | Not verified. Default `ARCH=riscv64` configure fails at `riscv64-unknown-elf-gcc not found`; no RISC-V target-toolchain or QEMU path was found locally, so SBI/DTB runtime coverage remains open. |

The checked-in i386 smoke harness requires the serial markers `APXH started.`, `NUX library (nux)`, userspace hello output, `SYSC0`/`SYSC6` pass messages, the `UCTXT_SETA2` kernel/user regression markers, `UADDR_VALIDRANGE test passed.`, `KVA_ALLOC_FREE test passed.`, and `User exited with error code: 42`. The checked-in amd64 smoke harness uses the standardized `x86_64-linux-gnu`/`i686-unknown-elf` override and additionally requires `IPI!`, all `SYSC0` through `SYSC6` pass messages, no unexpected kernel page fault, and repeated zero-valued `pnux_entry_pagefault` idle counter lines. Treat an rc 124 timeout as a pass only when the architecture's required markers appear before the timeout; otherwise investigate it as a failed or inconclusive runtime smoke.
