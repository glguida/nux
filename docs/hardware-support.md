...
| `amd64` | Yes (`configure.ac`) | `libhal_x86` amd64 | `libplt_acpi` | `apxh/configure` selects `multiboot efi` | Source has amd64 HAL, multiboot, EFI, ACPI, LAPIC/IOAPIC/HPET support. In this container, default configure is blocked on missing `amd64-unknown-elf-*`; an `x86_64-linux-gnu` prefix override with the reviewed i686 `TOOLCHAIN32` passes preflight/configure/build after the freestanding libec `-no-pie`/`-fno-pie` fixes. Bounded QEMU reaches APXH/NUX/tests, IPI, userspace, syscall, `UCTXT_SETA2`, exit, and idle counter markers; timeout rc 124 is expected after success because the demo keeps idling. |
...
| `amd64` | `/usr/bin/qemu-system-x86_64` is present from the same package, QEMU `10.0.8 (Debian 1:10.0.8+ds-0+deb13u1+b2)` | Override runtime verified; default target-toolchain still blocked. Default `ARCH=amd64` configure fails at missing default `amd64-unknown-elf-*` tools and default-PATH `i686-unknown-elf-gcc`; `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` passes preflight/configure/build with the stable external i386 `TOOLBIN` after the libec `-no-pie`/`-fno-pie` fixes. Bounded QEMU reaches APXH/NUX boot output, `IPI!`, userspace hello, `SYSC0`-`SYSC6`, `UCTXT_SETA2` kernel/user markers, `User exited with error code: 42`, and repeated `pnux_entry_pagefault 0`; timeout rc 124 is expected only after those success/idle markers. |
