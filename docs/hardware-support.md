...
The i386 serial-smoke markers observed in the verified run were `APXH started.`, `NUX library (nux)`, userspace hello output, `SYSC0`/`SYSC6` pass messages, the `UCTXT_SETA2` kernel/user regression markers, `UADDR_VALIDRANGE test passed.`, `KVA_ALLOC_FREE test passed.`, and `User exited with error code: 42`. Treat an rc 124 timeout as a pass only when those markers appear before the timeout; otherwise investigate it as a failed or inconclusive runtime smoke.
