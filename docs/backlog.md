...
7. **README boot-support claims are reconciled with configure behavior.**
   ...
   - Current status: README now describes configured APXH boot paths by architecture: i386 -> `multiboot`, amd64 -> `multiboot efi`, and riscv64 -> `sbi`. It also states that i386/multiboot is the full default-toolchain runtime-smoke path in this task environment, while the amd64 override path passes bounded QEMU with the reviewed `x86_64-linux-gnu`/`i686-unknown-elf` override after the libec PIE fixes. Default amd64 target tools and the RISC-V EFI platform contract remain unverified.
   - Follow-up trigger: reopen only if configure behavior changes or target-toolchain/runtime verification proves a different support claim. Default-toolchain amd64, amd64 EFI, and RISC-V EFI verification remain tracked in the architecture-specific follow-ups.
8. **amd64 override smoke is standardized; riscv64 SBI/DTB smoke is verified.**
   ...
   - Follow-up trigger: provision a deliberate real `amd64-unknown-elf` target toolchain only if default-prefix amd64 becomes a requirement. Otherwise keep the override harness reviewed and wire it into CI when the runner has host-prefixed x86_64 tools, the stable/approved i386 `TOOLBIN`, initialized submodules, `make`, and `qemu-system-x86_64`. Revisit RISC-V EFI only with an explicit ACPI-vs-DTB platform contract and linker/toolchain plan.
