...
5. **APXH configure architecture selection is fixed.**
   ...
   - Follow-up trigger: default `amd64` still needs real `amd64-unknown-elf-*`/default-PATH 32-bit target tools if default-toolchain support is desired. The local smoke policy now standardizes the reviewed `x86_64-linux-gnu`/`i686-unknown-elf` override and `tools/qemu-smoke-amd64.sh` captures it; wire that harness into CI only after the runner provides the required host-prefixed tools and QEMU. `riscv64` configure/build/QEMU verification still needs real target-toolchain paths and `qemu-system-riscv64`.
...
7. **README boot-support claims are reconciled with configure behavior.**
   ...
   - Current status: README now describes configured APXH boot paths by architecture: i386 -> `multiboot`, amd64 -> `multiboot efi`, and riscv64 -> `sbi efi`. It also states that i386/multiboot is the full default-toolchain runtime-smoke path in this task environment, while the amd64 override path passes bounded QEMU with the reviewed `x86_64-linux-gnu`/`i686-unknown-elf` override after the libec PIE fixes. Default amd64 target tools, riscv64, and the RISC-V EFI platform contract remain unverified.
   - Follow-up trigger: reopen only if configure behavior changes or target-toolchain/runtime verification proves a different support claim. Default-toolchain amd64, amd64 EFI, and riscv64 verification remain tracked in the architecture-specific follow-ups.
8. **amd64 override smoke is standardized; default amd64 and riscv64 remain toolchain-gated.**
   ...
   - amd64 status: default `ARCH=amd64` preflight/configure fails because default `amd64-unknown-elf-*` tools and default-PATH `i686-unknown-elf-gcc` are missing. A fresh architecture/toolchain follow-through found no real `amd64-unknown-elf-*`, `amd64-elf-*`, `x86_64-unknown-elf-*`, or `x86_64-elf-*` candidates in PATH, task workspaces, or common bin roots, and the README-recommended `gcc_toolchain_build` source tree was not present locally. Because the stable i386 GCC install is already about 2.0G and the fresh runner lacked `make`/host GCC/QEMU with `/home` 98% used, a task-local real amd64 GCC/binutils build was not low-risk.
   - Policy decision: use the reviewed `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` override as the supported local amd64 smoke path rather than waiting for a real default-prefix toolchain. `tools/qemu-smoke-amd64.sh` now runs that path from an out-of-tree build and accepts timeout rc 124 only after APXH/NUX, `IPI!`, userspace, `SYSC0` through `SYSC6`, `UCTXT_SETA2`, exit, no unexpected kernel page fault, and repeated zero-valued `pnux_entry_pagefault` idle counter evidence.
   ...
   - riscv64 status: default `ARCH=riscv64` preflight/configure fails at missing `riscv64-unknown-elf-*` tools, and fresh discovery found no local `riscv64-unknown-elf-*`, `riscv64-elf-*`, or `riscv64-linux-gnu-*` candidate and no `qemu-system-riscv64` path. Add a RISC-V target toolchain plus `qemu-system-riscv64` before riscv64 build/smoke follow-through.
   - Follow-up trigger: provision a deliberate real `amd64-unknown-elf` target toolchain only if default-prefix amd64 becomes a requirement. Otherwise keep the override harness reviewed and wire it into CI when the runner has host-prefixed x86_64 tools, the stable/approved i386 `TOOLBIN`, initialized submodules, `make`, and `qemu-system-x86_64`.
