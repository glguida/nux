...
9. **amd64 EFI OVMF smoke is verified from a clean worktree; dirty gnu-efi sources are an environment blocker.**
   ...
   - Current harness: after worktree submodule initialization, `PATH="$AMD64_TOOLBIN:$PATH" ARCH=amd64 ./tools/build-preflight.sh` passes. `tools/qemu-smoke-amd64-efi.sh` builds the EFI loader and payloads from an out-of-tree build, stages `EFI/BOOT/BOOTX64.EFI`, `kernel.elf`, and `user.elf`, auto-detects common OVMF paths or accepts `OVMF_CODE`/`OVMF_VARS`, and no longer fails on missing firmware in this container. With `TOOLBIN="$AMD64_TOOLBIN"` from a clean worktree, it reaches APXH/NUX, `IPI!`, userspace, `SYSC0` through `SYSC6`, `UCTXT_SETA2`, `User exited with error code: 42`, and repeated zero-valued `pnux_entry_pagefault` idle counters before the expected timeout. The earlier `libnux/alloc.h:192` ACPI/KVA assertion was fixed by commit `d9ba5f5256cb76c4f6b9207aec775a65d22e0f1a`.
