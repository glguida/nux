...
8. **amd64 default/override smokes are standardized; riscv64 SBI/DTB smoke is verified.**
   ...
   - Policy decision: use the README-built freestanding cache for default-prefix amd64 verification, keep compiler artifacts outside the repository, and rebuild/restore the external cache if default-path tools disappear. The reviewed `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` override remains a supported local smoke path rather than a silent default alias. `tools/qemu-smoke-amd64.sh` defaults to that override path and accepts timeout rc 124 only after APXH/NUX, `IPI!`, userspace, `SYSC0` through `SYSC6`, `UCTXT_SETA2`, exit, no unexpected kernel page fault, and repeated zero-valued `pnux_entry_pagefault` idle counter evidence.
...
9. **amd64 EFI build path is bounded; runtime smoke waits on firmware.**
   - Evidence: task `the-nux-amd64-efi-runtime-smoke` initialized submodules in a dedicated worktree and verified `ARCH=amd64 ./tools/build-preflight.sh` with the README-built `amd64-unknown-elf`/`i686-unknown-elf` cache. A fresh out-of-tree `ARCH=amd64` configure followed by `make -j1 -C apxh/efi all`, `make -j1 -C libhal_x86 all`, `make -j1 -C libplt_acpi all`, `make -j1 -C libnux all`, `make -j1 -C libnux_user all`, `make -j1 -C example/kern all`, and `make -j1 -C example/user all` produced `apxh/efi/apxh.efi`, `example/kern/example`, and `example/user/exuser`. `file(1)` identified `apxh.efi` as a PE32+ x86-64 EFI application.
   - Runtime blocker: `/usr/bin/qemu-system-x86_64` is present, but no OVMF/edk2 x86_64 firmware was found under `/usr/share` or `/usr/lib`; `dpkg-query` reports `ovmf` not installed. Local FAT-image helpers (`mformat`, `mcopy`, `mkfs.vfat`, `mkfs.fat`) are also absent, but the checked-in EFI harness uses QEMU's `fat:rw:` directory backend, so the first blocker is firmware.
   - Current harness: `tools/qemu-smoke-amd64-efi.sh` builds the EFI loader and payloads from an out-of-tree build, stages `EFI/BOOT/BOOTX64.EFI`, `kernel.elf`, and `user.elf`, auto-detects common OVMF paths or accepts `OVMF_CODE`/`OVMF_VARS`, and accepts timeout rc 124 only after the normal amd64 APXH/NUX/userspace serial markers. In the current container it fails early with `OVMF/edk2 x86_64 firmware not found` rather than silently skipping.
   - Next operator decision: install/provide a reviewed x86_64 OVMF/edk2 firmware asset outside the repository (for example the distro `ovmf` package or explicit `OVMF_CODE`/`OVMF_VARS` paths), then rerun `TOOLBIN=<README-built-cache>/install/bin ./tools/qemu-smoke-amd64-efi.sh`. Do not claim amd64 EFI runtime coverage until that harness reaches the required markers.
