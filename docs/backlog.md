...
5. **APXH configure architecture selection is fixed.**
   ...
   - Follow-up trigger: default `amd64` still needs real `amd64-unknown-elf-*`/default-PATH 32-bit target tools or a reviewed policy that standardizes the `x86_64-linux-gnu` override; amd64 still needs runtime page-fault debugging before a full smoke pass. `riscv64` configure/build/QEMU verification still needs real target-toolchain paths and `qemu-system-riscv64`.
...
7. **README boot-support claims are reconciled with configure behavior.**
   ...
   - Current status: README now describes configured APXH boot paths by architecture: i386 -> `multiboot`, amd64 -> `multiboot efi`, and riscv64 -> `sbi efi`. It also states that i386/multiboot is the full runtime-smoke path in this task environment, while amd64 has partial override build/QEMU coverage and riscv64 plus the RISC-V EFI platform contract remain unverified.
   - Follow-up trigger: reopen only if configure behavior changes or target-toolchain/runtime verification proves a different support claim. Full amd64 runtime-pass/default-toolchain and riscv64 verification remain tracked in the architecture-specific follow-ups.
8. **amd64/riscv64 build and smoke verification is bounded by default toolchains, the amd64 runtime fault, and riscv64 QEMU availability.**
   - Evidence: the 2026-05-30 follow-through integrated commit `51fc152c5d4cd92be9ee0ec9f7410e245bb16dd0` (`libec: disable PIE for freestanding links`) after review. Logs show `/usr/bin/qemu-system-x86_64` is present but no `qemu-system-riscv64`; no default `amd64-unknown-elf-*` or `riscv64-unknown-elf-*` target tools were on `PATH`; the stable external i686 `TOOLBIN` remains under the `the-nux-docs-capabilities` task workspace.
   - amd64 status: default `ARCH=amd64` preflight/configure fails because default `amd64-unknown-elf-*` tools and default-PATH `i686-unknown-elf-gcc` are missing. With `PATH` prepended by the stable i686 `TOOLBIN`, `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` preflight, configure, and `make` now pass. Worktree-local submodule initialization succeeded. Bounded QEMU reaches APXH/NUX boot output and regression markers including `UADDR_VALIDRANGE test passed` and `KVA_ALLOC_FREE test passed`, then hits the known `Unexpected Kernel Page Fault` before timeout; this is partial runtime coverage, not a full amd64 smoke pass.
   ...
   - Follow-up trigger: before adding an amd64 smoke harness or claiming a full amd64 runtime pass, debug the page fault and decide whether to provide a real amd64 target toolchain or standardize the reviewed `x86_64-linux-gnu` override. Add a RISC-V target toolchain plus `qemu-system-riscv64` before riscv64 build/smoke follow-through.
