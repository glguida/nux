...
5. **APXH configure architecture selection is fixed.**
   ...
   - Follow-up trigger: default `amd64` still needs real `amd64-unknown-elf-*`/default-PATH 32-bit target tools or a reviewed policy that standardizes the `x86_64-linux-gnu` override, and any checked-in amd64 smoke harness should be added as its own reviewed slice. `riscv64` configure/build/QEMU verification still needs real target-toolchain paths and `qemu-system-riscv64`.
...
8. **amd64/riscv64 build and smoke verification is bounded by default toolchains, override-policy decisions, and riscv64 QEMU availability.**
   - Evidence: the 2026-05-30 follow-through integrated commit `51fc152c5d4cd92be9ee0ec9f7410e245bb16dd0` (`libec: disable PIE for freestanding links`) after review, followed by commit `8e1a5365dbdb2277fe9a2853f272765cbc6dd98e` (`libec: disable PIE for freestanding compiles`). Logs show `/usr/bin/qemu-system-x86_64` is present but no `qemu-system-riscv64`; no default `amd64-unknown-elf-*` or `riscv64-unknown-elf-*` target tools were on `PATH`; the stable external i386 `TOOLBIN` remains under the `the-nux-docs-capabilities` task workspace.
   - amd64 status: default `ARCH=amd64` preflight/configure fails because default `amd64-unknown-elf-*` tools and default-PATH `i686-unknown-elf-gcc` are missing. With `PATH` prepended by the stable i386 `TOOLBIN`, `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` preflight, configure, and `make` pass. Worktree-local submodule initialization succeeded. The earlier bounded QEMU failure was traced to host GCC default-PIE code generation in freestanding fixed-address objects; after the compile-side `-fno-pie` fix, bounded QEMU reaches APXH/NUX boot output, `IPI!`, userspace hello, `SYSC0` through `SYSC6`, `UCTXT_SETA2` kernel/user markers, `User exited with error code: 42`, and repeated `pnux_entry_pagefault 0`. Timeout rc 124 is expected only after those success/idle markers because the example does not shut QEMU down.
   - i386 regression status: the same follow-up verification reran `tools/qemu-smoke-i386.sh` with the stable external i386 `TOOLBIN`; it returned rc 0/PASS.
   - riscv64 status: default `ARCH=riscv64` preflight/configure fails at missing `riscv64-unknown-elf-*` tools, and no local `riscv64-elf-*` or `riscv64-linux-gnu-*` candidate and no `qemu-system-riscv64` path were found, so build/smoke was not attempted.
   - Follow-up trigger: before adding a checked-in amd64 smoke harness or claiming default-toolchain amd64 support, decide whether to provide a real amd64 target toolchain or standardize the reviewed `x86_64-linux-gnu` override. Add a RISC-V target toolchain plus `qemu-system-riscv64` before riscv64 build/smoke follow-through.
