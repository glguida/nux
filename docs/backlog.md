...
8. **amd64/riscv64 build and smoke verification is bounded by toolchain, QEMU, and workspace capacity.**
   - Evidence: the 2026-05-30 `the-nux-amd64-riscv-smoke` probe found `/usr/bin/qemu-system-x86_64` but no `qemu-system-riscv64`; no default `amd64-unknown-elf-*` or `riscv64-unknown-elf-*` target tools were on `PATH`; the only project-local target toolchain found was the reviewed i686 `TOOLBIN` under the `the-nux-docs-capabilities` task workspace.
   - amd64 status: default `ARCH=amd64` configure fails at `amd64-unknown-elf-gcc not found`. With `PATH` prepended by the stable i686 `TOOLBIN`, `ARCH=amd64 TOOLCHAIN=x86_64-linux-gnu TOOLCHAIN32=i686-unknown-elf` configures successfully through top-level, APXH, and example. The follow-on build does not reach QEMU; it stops in submodule-dependent `tools/libbfd`/APXH EFI work, and a submodule initialization attempt in the dedicated worktree failed with `No space left on device` while checking out `contrib/binutils`.
   - riscv64 status: default `ARCH=riscv64` configure fails at `riscv64-unknown-elf-gcc not found`; no local `riscv64-elf-*` or `riscv64-linux-gnu-*` candidate and no `qemu-system-riscv64` path were found, so build/smoke was not attempted.
   - Follow-up trigger: before adding amd64/riscv64 smoke harnesses, provide either a real amd64 target toolchain or an explicit reviewed decision that `x86_64-linux-gnu` is an acceptable amd64 tool prefix, ensure enough free workspace for checked-out submodules and submodule-local build products, and add a RISC-V target toolchain plus `qemu-system-riscv64` for riscv64.
