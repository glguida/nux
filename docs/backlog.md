...
9. **amd64 EFI build and OVMF launch are bounded; runtime smoke blocks on ACPI/KVA.**
   ...
   - Current harness: after worktree submodule initialization, `PATH="$AMD64_TOOLBIN:$PATH" ARCH=amd64 ./tools/build-preflight.sh` passed. `tools/qemu-smoke-amd64-efi.sh` builds the EFI loader and payloads from an out-of-tree build, stages `EFI/BOOT/BOOTX64.EFI`, `kernel.elf`, and `user.elf`, auto-detects common OVMF paths or accepts `OVMF_CODE`/`OVMF_VARS`, and no longer fails on missing firmware in this container. With `TOOLBIN="$AMD64_TOOLBIN"`, it reaches `APXH started.` and `NUX library (nux)` under OVMF, then fails before the IPI/userspace/syscall/exit/idle markers on `Assertion 'diff >= 0' failed at libnux/alloc.h:192` in `kva_alloc -> kva_physmap -> load_table -> acpi_init -> plt_init`. Explicit split firmware and a manual monolithic `-bios /usr/share/ovmf/OVMF.fd` run reproduce the same assertion.
   - Next code/runtime decision: investigate and fix the amd64 EFI ACPI/KVA allocator assertion under OVMF. Do not claim amd64 EFI runtime coverage until that bounded fix lands and the checked-in harness reaches the required markers.
