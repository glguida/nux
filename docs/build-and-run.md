...
the `UMAP_BOUNDS` and `KMAP_UPDATE` markers, `User exited with error code: 42`, no unexpected kernel page fault, and repeated
...
and `UADDR_MEMSET` kernel/user markers, the `UMAP_BOUNDS` and `KMAP_UPDATE` markers,
`User exited with error code: 42`, and repeated
...
`UMAP_BOUNDS`, `KVA_ALLOC_FREE`, `KMAP_UPDATE`, `User exited with error code: 42`, and repeated zero-valued
...
- The baseline amd64 QEMU failure was traced to host GCC default-PIE code generation in freestanding fixed-address objects. With the override path after commit `8e1a5365dbdb2277fe9a2853f272765cbc6dd98e`, the bounded amd64 QEMU smoke reaches APXH/NUX boot output, `IPI!`, `Hello from userspace, NUX!`, `SYSC0` through `SYSC6`, `UCTXT_SETA2 test passed.`, `UCTXT_SETA2 user test passed.`, `UADDR_MEMSET test passed.`, `UADDR_MEMSET user test passed.`, `UMAP_BOUNDS test passed.`, `KMAP_UPDATE test passed.`, `User exited with error code: 42`, and repeated zero-valued `pnux_entry_pagefault` idle counter lines; timeout rc 124 is expected only after those success/idle markers.
...
- The verified riscv64 runtime path is the explicit SBI/DTB subset build used by `tools/qemu-smoke-riscv64.sh`; bounded QEMU reaches OpenSBI/APXH/NUX/userspace/syscall/UCTXT/UADDR/UMAP/KVA markers and repeated zero-valued `pnux_entry_pagefault` idle counters before the expected timeout.
