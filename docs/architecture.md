...
The HAL abstracts CPU instructions, interrupt frames, page-table leaf entries, TLB operations, virtual-memory areas, physical-memory descriptions, and CPU bring-up. The interface is declared in `include/nux/hal.h`. Current tracked code exposes leaf-level `hal_l1p_t`/`hal_l1e_t` operations. A historical task-log/base-checkout note mentioned future `ROOTPTE`/`ROOTPTEP` and `LEAFPTE`/`LEAFPTEP` names, but the generic PTE audit treats that as uncertain design input rather than an implementation mandate; the tracked leaf API remains authoritative until a source-backed generic NUX problem justifies a change.
...
- RISC-V code is in `libhal_riscv/riscv.c`, `libhal_riscv/pmap.c`, `libhal_riscv/sv48.c`, and `libhal_riscv/entry.S`. The interrupt-return path preserves the current `sstatus` value except for the frame-controlled SSTATUS `SPIE`/`SPP` bits restored for `sret`.
...
- x86 supervisor hardening now enables CPUID-leaf-7-gated SMEP and SMAP per CPU from the i386/amd64 `hal_pcpu_enter()` paths. User-copy windows still use guarded `stac`/`clac` only when SMAP is active, user-origin entry clears AC, and the reviewed AP-bootstrap audit keeps reset-vector/trampoline mappings supervisor-only (no `PTE_U`) with forced-SMEP smoke coverage.
...
- APXH boot-path verification is still incomplete beyond the reviewed i386/multiboot, amd64 multiboot default-prefix/override, amd64 EFI, and riscv64 SBI/DTB paths: amd64 EFI now has default-prefix OVMF runtime smoke coverage through the normal APXH/NUX, IPI, userspace, syscall, exit, and idle markers from a clean worktree; configured RISC-V remains on the APXH SBI/DTB (`PLT_DTB`) path, while RISC-V EFI is explicitly guarded unsupported because the current APXH EFI descriptor path publishes `PLT_ACPI`; real RISC-V EFI support needs a future reviewed descriptor and linker/toolchain contract; see [hardware support](hardware-support.md) and [backlog](backlog.md).
