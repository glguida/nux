...
- APXH boot-path verification is still incomplete beyond the reviewed i386/multiboot, amd64 multiboot default-prefix/override, and riscv64 SBI/DTB paths: amd64 EFI now has a verified default-prefix build and OVMF launch to APXH/NUX, but runtime coverage is blocked by the ACPI/KVA allocator assertion before the normal amd64 markers; RISC-V EFI still needs a platform-contract/toolchain decision; see [hardware support](hardware-support.md) and [backlog](backlog.md).
