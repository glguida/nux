...
- The HAL UMAP leaf lookup contract rejects non-user addresses with `L1P_INVALID`; the RISC-V implementation now matches x86 for addresses below the UMAP minimum or at/above the UMAP maximum, and the example smoke prints `UMAP_BOUNDS test passed.` after checking the first one-past-user address against both a bootstrapped UMAP and the current CPU UMAP.
